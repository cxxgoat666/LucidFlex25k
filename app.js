const RULES = {
  ...
  consistencyCushionAtTarget: 650,
  maxMini: 2,
  maxMicro: 20,
  minTradingDays: 2,
...
function getMetrics() {
  ...
  const cushionLimit = accountProfit >= RULES.profitTarget ? Math.max(RULES.consistencyLimit * accountProfit, RULES.consistencyCushionAtTarget) : RULES.consistencyLimit * Math.max(accountProfit, RULES.profitTarget);
...
function buildObjectives(metrics) {
  const objectives = [
    ...
    },
    {
      ...
      detail: `Largest winning day is ${currency.format(metrics.largestProfitDay)}; cushion is ${currency.format(metrics.cushionLimit)}`,
...
function renderCalendar(metrics) {
  ...
  const monthWinDays = monthKeys.filter((key) => (metrics.dayPnlMap.get(key) || 0) > 0).length;
  const monthWinRate = monthKeys.length ? monthWinDays / monthKeys.length : 0;
  els.calendarWinRate.textContent = `${formatPercent(monthWinRate)} daily win rate`;
